...
def generate_combined_graph(scores_by_question):
    ...
    fig, ax = plt.subplots(figsize=(len(dates) * 1.5, 6))  # Adjust figure width based on dates

    # Fixed bar width and spacing
    bar_width = 20 / fig.dpi  # 20px converted to figure units
    x_positions = np.arange(len(dates)) * (bar_width * len(question_ids) + 0.1)  # Space between dates
