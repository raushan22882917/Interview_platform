    ...
    if request.method == 'POST':
        ...
        if session['company_name'] is not None:
            if 'answer' in request.form:
                ...
                return "Error: Missing answer", 400  # Handle missing answer

    # Show the current question and answers
    return render_template('HR_Round/index.html', company_name=session['company_name'], 
...
def store_answer_in_db(company_name, question, answer, feedback, score):
    try:
        ...
        insert_query = sql.SQL("""
            INSERT INTO user_answers_hr (company_name, question, answer, feedback, score)
            VALUES (%s, %s, %s, %s, %s)
        """)
