...
paypalrestsdk.configure({
    ...
    "client_id": "AVS2QpD2wSVL_nnvCKw7wv5f_j6M54aQwKH3pA78YJSZDZw7r-GulWhh43YwOsYiuG2gKTmdBtdJFXgH",
    "client_secret": "EE5-DnMV01JDDaybxrBNb_6SNXnmcnLufgzQZ5H1fmy3o6XGqRFcHOfu5sljXIGYw9z39qOgMpkQCCCs"
})
...
# Route to display PayPal payment page
@app.route('/payment')
def payment_page():
...
def pay():
    ...
    user_email = session['email']
    subscription_type = request.form['subscription_type']  # Get subscription choice (1 month, 3 months, etc.)
    
    amount = {'1': 0.1, '3': 0.2, '6': 0.3, '12': 0.4}[subscription_type]  # Amount based on subscription
    ...
    payment = paypalrestsdk.Payment({
        ...
        "transactions": [{
            ...
            "description": f"Subscription for {subscription_type} month(s)"
        }]
    })
...
def payment_success():
    ...
    # Mark subscription as active in DB (update user subscription details)
    user_email = session['email']
    ...
    cursor.execute(
        """
        UPDATE subscriptions SET subscription_status = 'active', end_date = CURRENT_TIMESTAMP + INTERVAL '%s month'
        WHERE user_email = %s
        """,
        (subscription_type, user_email)
    )
    connection.commit()
    cursor.close()
    connection.close()

    return jsonify({'message': 'Payment successful. Your subscription has been activated.'})

# Route for canceled payment
@app.route('/payment_cancel')
def payment_cancel():
    return jsonify({'message': 'Payment was canceled. Please try again.'})

# Route to submit an answer (with subscription check)
...
# Route to display subscription page if user has exceeded free questions
@app.route('/subscribe')
def subscribe():
