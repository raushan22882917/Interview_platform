...
@app.route("/hr-round", methods=["GET", "POST"])
def hr_round():
    if request.method == "POST":
        # Getting form data
        ...
        # Debugging: print form data to check if they're captured
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute("""
                INSERT INTO questions (question_text, question_type, difficulty_level, category, subtopic, job_role, experience_level)
                VALUES (%s, %s, %s, %s, %s, %s, %s)
            """, (question_text, question_type, difficulty_level, category, subtopic, job_role, experience_level))
            conn.commit()
            cur.close()
            conn.close()
    ...
    return render_template("hr_round_form.html", email=session.get("email"))

    # If GET request, render the form
    return render_template("publish/form.html", email=session.get("email"))
