...
# Check subscription status
    remaining_days = check_subscription(user_email)
    
    
    cursor.execute("SELECT email, amount, date, months FROM payments WHERE email = %s", (user_email,))
    payments = cursor.fetchall()
    
    
    rows = get_user_data(user_email)

    # Generate PDFs
    generate_pdfs(rows)

    # Create the attempts list with full score for each attempt
    attempts = []
    total_score = 0
    num_rows = len(rows)
    
    dates = []  # Store the dates for each attempt
    total_scores = []  # Store the total scores for each attempt
    average_scores_hr = []  # Store the average scores for each attempt (calculated per hour)

    for i in range(0, num_rows, 10):
        # Extract 10 rows for this attempt
        attempt_rows = rows[i:i + 10]
        
        # Calculate total score for this attempt
        attempt_total_score = sum(row[4] for row in attempt_rows)
        
        # Append the attempt data
        attempt_avg_score_hr = round(attempt_total_score / len(attempt_rows), 2) if len(attempt_rows) > 0 else 0
        attempts.append({
            "id": len(attempts) + 1,
            "file": f"Attempt_{len(attempts) + 1}.pdf",
            "total_score": attempt_total_score,
            "average_score": attempt_avg_score_hr,
            "date": datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        })

        # Collect data for the graph
        dates.append(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
        total_scores.append(attempt_total_score)
        average_scores_hr.append(attempt_avg_score_hr)

    # Calculate total and average score for all attempts
    total_score = sum(attempt["total_score"] for attempt in attempts)
    average_score = round(total_score / len(rows), 2) if len(rows) > 0 else 0
    ...
    return render_template('profile_details.html',
        ...
        average_score_r=average_score,
        dates=dates,
        total_scores=total_scores,
        average_scores_hr=average_scores_hr,
                           average_score_last_3=average_score_last_3,remaining_days=remaining_days,payments=payments,
...
def generate_combined_graph(scores_by_question):
    ...
    """
    # Prepare data
    dates = sorted({date for scores in scores_by_question.values() for date, _ in scores})
    question_ids = sorted(scores_by_question.keys())

    if not dates or not question_ids:  # Avoid empty data issues
        return "<p>No data available to generate the graph.</p>"

    # Aggregate scores by question_id and date
    scores_by_date = defaultdict(lambda: [0] * len(question_ids))
    for qid, values in scores_by_question.items():
        for date, score in values:
            try:
                score = float(score)  # Ensure the score is a float
                if np.isfinite(score):  # Check for valid numerical scores
                    scores_by_date[date][question_ids.index(qid)] = score
    ...
    fig, ax = plt.subplots(figsize=(len(dates) * 1.5, 6))

    # Fixed bar width and spacing
    bar_width = 0.3  # Adjusted width
    x_positions = np.arange(len(dates)) * (bar_width * len(question_ids) + 0.2)

    color_palette = plt.cm.get_cmap('tab20', len(question_ids))
    ...
    # Plot line graph
    ax.plot(
        x_positions + (len(question_ids) - 1) * bar_width / 2,
        avg_scores,
        color="red",
        marker="o",
        label="Average Score",
    ...
    # Labels and titles
    ax.set_xticks(x_positions + (len(question_ids) - 1) * bar_width / 2)
...
# Function to get the current attempt count for the user
...
def submit_answer_take():
    data = request.json
    user_answer = data.get('user_answer', None)  # Safe access with .get()
    ...
    if current_index < len(selected_questions):
        ...
        try:
            df = pd.read_csv(csv_file)
            row = df[df['Question'] == question].iloc[0]  # Ensure row is found
        except IndexError:  # In case the question isn't found
            return jsonify({'error': f"Question '{question}' not found in the CSV."}), 404
        except Exception as e:
        ...
        best_similarity = max(web_similarity, gpt35_similarity, gpt4_similarity)
...
def admin():
    ...
    if request.method == 'POST':
        ...
        # Handle payment form submission
            ...
            reactivation_date = request.form['reactivation_date'] if status == 'inactive' else None
            ...
            cur = conn.cursor()
            cur.execute("""
                INSERT INTO payments (email, amount, date, months, status, reactivation_date)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (email, amount, date, months, status, reactivation_date))
            conn.commit()
            cur.close()
            conn.close()
    ...
    if count > 2:
        ...
        if subscription:
            ...
            if isinstance(subscription_date, datetime):
                ...
                subscription_date = datetime.combine(subscription_date, datetime.min.time())  # Ensure correct datetime format

            months = subscription['months']
            expiration_date = subscription_date + timedelta(days=months * 30)  # Approximate days per month
            today = datetime.today()

            if expiration_date >= today:
                remaining_days = (expiration_date - today).days  # Calculate remaining days
...
def Hr_home():
    ...
    if subscription_status == 'active':
        return render_template('HR_Round/home.html', button_message="Start", remaining_days=remaining_days)
    elif subscription_status == 'expired':
        return render_template('HR_Round/home.html', button_message="Renew Subscription")
    elif subscription_status == 'free_trial':
        return render_template('HR_Round/home.html', button_message="Free Trial")
    else:
        return render_template('HR_Round/home.html', button_message="Subscribe Now")
...
@app.route('/subscribe')
def subscribe():
    return render_template('subscribe.html')  # Ensure .html extension is included
...
def get_user_data(user_email):
    ...
    cursor = conn.cursor()
    query = """
        SELECT company_name, question, user_answer, feedback, score
        FROM interview_data
        WHERE user_email = %s
    """
...
# Function to generate user-specific PDFs
def generate_pdfs(rows, rows_per_pdf=10):
    # Clear old PDFs
    for file in os.listdir(PDF_FOLDER):
        os.remove(os.path.join(PDF_FOLDER, file))

    pdf_index = 1
    attempts = []  # Initialize the attempts list to store attempt info

    for i in range(0, len(rows), rows_per_pdf):
        pdf = FPDF()
        ...
        pdf.cell(0, 10, f"Interview Data - Attempt {pdf_index}", ln=True, align="C")
        pdf.ln(5)  # Slightly smaller break for title

        # Set font for content
        pdf.set_font("Arial", size=12)

        total_score = 0  # Initialize total score for this attempt

        for row in rows[i:i + rows_per_pdf]:
            company_name, question, user_answer, feedback, score = row

            # Add data in a readable format
            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, "Company Name:", ln=True)
            pdf.set_font("Arial", size=12)
            pdf.multi_cell(0, 4, clean_text(company_name))  # Adjust line height to fit tighter spacing
            pdf.ln(2)  # 2px spacing

            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, "Question:", ln=True)
            pdf.set_font("Arial", size=12)
            pdf.multi_cell(0, 4, clean_text(question))
            pdf.ln(2)  # 2px spacing

            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, "Answer:", ln=True)
            pdf.set_font("Arial", size=12)
            pdf.multi_cell(0, 4, clean_text(user_answer))
            pdf.ln(2)  # 2px spacing

            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, "AI Feedback:", ln=True)
            pdf.set_font("Arial", size=12)
            pdf.multi_cell(0, 4, clean_text(feedback))
            pdf.ln(2)  # 2px spacing

            pdf.set_font("Arial", style="B", size=12)
            pdf.cell(0, 10, "Score:", ln=True)
            pdf.set_font("Arial", size=12)
            pdf.cell(0, 4, str(score), ln=True)
            pdf.ln(2)  # 2px spacing

            # Accumulate the score for this attempt
            total_score += score
            ...
            pdf.ln(2)  # 2px spacing after line

        # Save the PDF
        pdf_path = os.path.join(PDF_FOLDER, f"Attempt_{pdf_index}.pdf")
        pdf.output(pdf_path)

        # Calculate average score for the attempt
        num_rows = len(rows[i:i + rows_per_pdf])
        average_score = total_score / num_rows if num_rows > 0 else 0

        # Add attempt info for display in the HTML page
        attempt_info = {
            "id": pdf_index,
            "file": f"Attempt_{pdf_index}.pdf",
            "total_score": score,
            "average_score": average_score,
            "date": datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # Current date
        }
...
client = Groq(api_key=os.environ.get("GROQ_API_KEY", "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"))
...
def start():
    ...
    return redirect('/step')  # Redirect to the first step

@app.route('/step', methods=['GET', 'POST'])
def step():
    # Get current step from session or default to the first step
    current_step = session.get('current_step', STEPS[0])
    responses = session.get('responses', {})

    # Initialize session['marks'] if not already present
    ...
    if request.method == 'POST':
        ...
        # Store feedback and score for the current step
        ...
        if current_index < len(STEPS) - 1:
            ...
            return redirect('/step')  # Go to the next step
        else:
            # Calculate total score when all steps are completed
            total_score = sum(session['marks'].values())
            return render_template('Ai/feedback.html', responses=responses, total_score=total_score, get_feedback=get_feedback, get_feedback_class=get_feedback_class)

    # Render the current step
    return render_template('Ai/steps.html', step=current_step, question=session.get('question'), responses=responses, get_feedback=get_feedback, get_feedback_class=get_feedback_class)
def process_step(step, user_input):
    """Process user input for a step and return feedback and score."""
    question = session['question']
    prompt = generate_prompt(step, question, user_input)
    feedback = call_groq_api(prompt)
    score = extract_score_from_feedback(feedback)

    return feedback, score

def generate_prompt(step, question, user_input):
    """Generate tailored prompts for each step, with evaluation criteria and qualitative feedback guidance."""
    
    # Step 1: Example Evaluation
    if step == "Example":
        return (
            f"### Task: Evaluate Examples\n"
f"**Question:** {question}\n"
f"**Provided Examples:**\n{user_input}\n\n"
f"#### Evaluation Criteria:\n"
f"- **Number of Examples:** Ensure that there are at least 3 input-output pairs provided in `{user_input}`.\n"
f"- **Diversity:** Check if the examples in `{user_input}` cover a variety of scenarios and edge cases.\n"
f"- **Clarity:** Evaluate whether the examples in `{user_input}` are clear and easy to understand.\n"
f"- **Relevance:** Confirm that the examples in `{user_input}` are directly related to the question and problem.\n\n"
f"#### Feedback:\n"
f"- **Number of Examples Feedback:** Provide feedback on whether the quantity of examples in `{user_input}` is sufficient.\n"
f"- **Diversity Feedback:** Assess the diversity of the examples in `{user_input}`, checking for coverage of various situations.\n"
f"- **Clarity Feedback:** Comment on the clarity and comprehensibility of the provided examples in `{user_input}`.\n"
f"- **Relevance Feedback:** Evaluate how well the examples in `{user_input}` align with the question and the context.\n"
f"- **Overall Rating:** Provide a qualitative rating (Excellent, Very Good, Good, Needs Improvement).\n\n"
f"#### Explanation:**\n"
f"- Break down each criterion and explain how the provided examples `{user_input}` meet or deviate from expectations."

        )
    
    # Step 2: Solution Approach Evaluation
    elif step == "Solution":
        return (
f"### Task: Assess Solution Approach\n"
f"**Question:** {question}\n"
f"**Proposed Solution:**\n{user_input}\n\n"
f"#### Evaluation Criteria:\n"
f"- **Correctness:** Check if the proposed solution `{user_input}` correctly addresses the problem and requirements.\n"
f"- **Feasibility:** Evaluate whether the solution is practical, considering constraints and real-world implementation.\n"
f"- **Logical Flow:** Assess the flow and organization of the solution approach `{user_input}` to ensure it is coherent and easy to follow.\n"
f"- **Completeness:** Ensure that all parts of the problem are addressed by the solution, and no critical aspects are left out.\n\n"
f"#### Feedback:\n"
f"- **Correctness Feedback:** Provide feedback on whether the solution correctly addresses the problem and meets the given requirements.\n"
f"- **Feasibility Feedback:** Comment on the practicality of the solution and any constraints that may affect its implementation.\n"
f"- **Logical Flow Feedback:** Provide insights on the clarity, structure, and readability of the solution approach `{user_input}`.\n"
f"- **Completeness Feedback:** Offer feedback on whether all aspects of the problem are covered in the proposed solution.\n"
f"- **Overall Rating:** Provide a qualitative rating (Excellent, Very Good, Good, Needs Improvement).\n\n"
f"#### Explanation:**\n"
f"- Break down each criterion and explain how the proposed solution `{user_input}` aligns or deviates from expectations."

        )
    
    # Step 3: Test Case Coverage
    elif step == "Test Cases":
        return (
           f"### Task: Review Test Cases\n"
f"**Question:** {question}\n"
f"**Test Cases Provided:**\n{user_input}\n\n"
f"#### Evaluation Criteria:\n"
f"- **Coverage:** Check whether the test cases in `{user_input}` account for all possible scenarios, including edge cases.\n"
f"- **Completeness:** Evaluate if the test cases in `{user_input}` sufficiently test the solution and cover both expected and unexpected inputs.\n\n"
f"#### Feedback:\n"
f"- **Coverage Feedback:** Provide feedback on whether the test cases cover all scenarios and edge cases.\n"
f"- **Completeness Feedback:** Comment on whether the test cases adequately validate the solution’s correctness.\n"
f"- **Improvement Suggestions:** Offer specific suggestions to enhance test case coverage and completeness.\n"
f"- **Overall Rating:** Provide a qualitative rating (Excellent, Very Good, Good, Needs Improvement).\n\n"
f"#### Explanation:**\n"
f"- Include a detailed breakdown of how the test cases `{user_input}` align with the expected coverage and completeness."
...
f"**User's Code:**\n{user_input}\n\n"
f"#### Evaluation Criteria:\n"
f"- **Correctness:** Verify whether the user's code `{user_input}` solves the problem as expected.\n"
f"- **Time Complexity:** Assess the efficiency of the solution based on the provided input `{user_input}`.\n"
f"- **Space Complexity:** Check the memory usage and ensure the analysis of `{user_input}` is accurate.\n"
f"- **Improvements:** Suggest optimizations or highlight best practices related to `{user_input}`.\n\n"
f"#### Feedback:\n"
f"- **Correctness Feedback:** Provide feedback on whether the code `{user_input}` meets the problem requirements.\n"
f"- **Time Complexity Feedback:** Evaluate and comment on the efficiency of `{user_input}`.\n"
f"- **Space Complexity Feedback:** Discuss the accuracy of the memory usage assessment.\n"
f"- **Improvement Suggestions:** Offer specific suggestions for improving the code `{user_input}`, including optimizations and best practices.\n"
f"- **Overall Rating:** Provide a qualitative rating (Excellent, Very Good, Good, Needs Improvement).\n\n"
f"#### Explanation:**\n"
f"- Include a breakdown of each criterion and how `{user_input}` aligns or deviates from the expected standards."
    ...
    # Step 5: Validation Feedback
        ...
        return (
           f"### Task: Validation Review\n"
            f"**Question:** {question}\n"
            f"**User Submission:**\n{user_input}\n\n"
            f"#### Evaluation Criteria:\n"
            f"- **Time Complexity:** Verify whether the time complexity analysis of `{user_input}` is correct and justified.\n"
            f"- **Space Complexity:** Check if the space complexity for `{user_input}` is accurate and explained properly.\n"
            f"- **Test Case Results:** Confirm that `{user_input}` passes all public and private test cases.\n"
            f"- **Code Quality & Readability:** Evaluate the clarity, comments, and structure of the submission `{user_input}`.\n\n"
            f"#### Feedback:\n"
            f"- **Time Complexity Feedback:** Provide specific feedback based on whether the user's time complexity analysis was accurate or needs improvement.\n"
            f"- **Space Complexity Feedback:** Comment on the correctness and explanation of the space complexity.\n"
            f"- **Test Case Feedback:** Indicate whether the solution passes the test cases and suggest improvements if needed.\n"
            f"- **Code Quality Feedback:** Offer suggestions for improving code clarity, style, and organization.\n"
            f"- **Overall Rating:** Provide a qualitative rating (Excellent, Very Good, Good, Needs Improvement).\n\n"
            f"#### Explanation:**\n"
            f"- Break down how `{user_input}` aligns or deviates from expectations in each criterion."
...
def extract_score_from_feedback(feedback):
    """Extract score based on feedback sentiment."""
    if "Excellent" in feedback:
        ...
        return 0  # Default for unclear/poor feedback
def get_feedback(score):
    if score == 5:
        ...
        return "Needs Improvement"  # Default
def get_feedback_class(score):
    if score == 5:
