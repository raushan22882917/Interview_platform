...
def step():
    ...
    if request.method == 'POST':
        ...
        session['marks'][current_step] = score
...
def process_step(step, user_input):
    ...
    # Check if feedback is needed (if the input is incorrect)
    if "incorrect" in feedback.lower() or "improvement" in feedback.lower() or "error" in feedback.lower():
        is_feedback_needed = True

    score = extract_score_from_feedback(feedback) if is_feedback_needed else 5  # Assume 5 if no issues

    return feedback if is_feedback_needed else "", score

def generate_prompt(step, question, user_input):
    """Generate tailored prompts for each step."""
    if step == "Example":
        return f"Question: {question}\nExamples: {user_input}\nCheck for at least 3 input-output pairs and assess diversity."
    elif step == "Solution":
        return f"Question: {question}\nSolution approach: {user_input}\nEvaluate based on logic, feasibility, and correctness."
    elif step == "Test Cases":
        return f"Question: {question}\nTest cases: {user_input}\nCheck for completeness, including edge cases."
    elif step == "Code":
        return f"Question: {question}\nCode: {user_input}\nEvaluate correctness, time complexity, and space complexity."
    elif step == "Validation":
        return f"Question: {question}\nValidation: {user_input}\nRun test cases and check for time and space complexity."

def extract_score_from_feedback(feedback):
    """Extract score based on feedback sentiment."""
    if "excellent" in feedback.lower():
        return 5
    elif "very good" in feedback.lower():
        return 4
