...
def hr_round():
    ...
    # GET request: fetch questions from the database and display them
    user_email = session.get('email')
    if not user_email:
        flash("User is not logged in.", "danger")
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT * FROM intr_questions WHERE user_email = %s', (user_email,))
    questions = cur.fetchall()
    cur.close()
    conn.close()

    # Pass questions to the template to display them
    return render_template('publish/form.html', email=user_email, questions=questions)
