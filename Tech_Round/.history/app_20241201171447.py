...
def get_data():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT company_name, question, user_answer, feedback, score FROM interview_data")
...
def generate_pdfs(rows, rows_per_pdf=10):
    for file in os.listdir(PDF_FOLDER):  # Clear old PDFs
    ...
    for i in range(0, len(rows), rows_per_pdf):
        ...
        for row in rows[i:i + rows_per_pdf]:
            ...
            pdf.cell(40, 10, company_name, border=1)
            pdf.cell(80, 10, question, border=1)
            pdf.cell(30, 10, user_answer, border=1)
            pdf.cell(30, 10, feedback, border=1)
