...
def start():
    ...
    session['responses'] = {}
    return redirect(url_for('step'))  # Updated redirect with url_for
...
def step():
    ...
    # Move to the next step if there is any
    current_index = STEPS.index(current_step)
    if current_index < len(STEPS) - 1:
        session['current_step'] = STEPS[current_index + 1]
        return jsonify({'next_step': STEPS[current_index + 1], 'feedback': feedback, 'responses': responses})

    return jsonify({'feedback': "Thank you for completing all steps. Here is the full feedback.", 'responses': responses})
