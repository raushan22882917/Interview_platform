...
# Check subscription status
    ...
    # Create the attempts list
    attempts = [{"id": i + 1, "file": f"Attempt_{i + 1}.pdf"} for i in range(len(rows) // 10 + (1 if len(rows) % 10 else 0))]

    # Calculate total and average score
    attempt_score = sum(row[4] for row in rows)  # Sum of all the scores in the rows
    avg_score_hr = round(attempt_score / len(rows), 2) if len(rows) > 0 else 0  # Average score
