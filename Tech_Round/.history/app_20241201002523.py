...
def submit_answer():
    ...
    score = CATEGORY_SCORES.get(feedback.split()[0], 2)  # Use first word of feedback to determine score
...
def payment():
    ...
    payment = paypalrestsdk.Payment({
        ...
        "transactions": [{
            ...
            "description": f"Subscription for {amount} USD"
        }]
    })
...
def execute():
    ...
    if payment.execute({'payer_id': request.form['payerID']}):
        ...
        user_email = session.get('email')  # Ensure user is logged in
        store_payment_in_db(user_email, payment.id, payment.transactions[0].amount.total)

        # Update user's subscription and allow unlimited attempts for the day
        update_user_subscription(user_email)
...
def store_payment_in_db(user_email, payment_id, amount):
    connection = get_db_connection()
    cursor = connection.cursor()
    
    cursor.execute("""
        INSERT INTO user_payments (user_email, payment_id, amount)
        VALUES (%s, %s, %s)
    """, (user_email, payment_id, amount))
    
    connection.commit()
    cursor.close()
    connection.close()

# Function to update user's subscription in the database
def update_user_subscription(user_email):
    connection = get_db_connection()
    cursor = connection.cursor()

    cursor.execute("""
        UPDATE users SET subscription_status = TRUE, attempts = 0
        WHERE email = %s
    """, (user_email,))
...
# Route to check user's remaining attempts (for free users)
@app.route('/check_attempts')
def check_attempts():
    user_email = session.get('email')
    connection = get_db_connection()
    cursor = connection.cursor()
    
    cursor.execute("SELECT attempts, subscription_status FROM users WHERE email = %s", (user_email,))
    result = cursor.fetchone()
    
    if result:
        attempts, subscription_status = result
        if subscription_status or attempts < 2:
            return jsonify({'message': 'You have remaining attempts.'})
        else:
            return jsonify({'message': 'You have used all attempts. Please subscribe to continue.'})
    else:
        return jsonify({'message': 'User not found. Please log in.'})

# Route to display subscription plans
