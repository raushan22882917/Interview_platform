...
# Route to show the form and the saved payments
@app.route('/admin')
def index():
    # Connect to the database and fetch the saved payments
...
def save_payment():
    ...
    # Connect to the database and insert the payment data
    conn = psycopg2.connect(**conn_params)
    cursor = conn.cursor()
    cursor.execute("""
    ...
    if action == 'save':
        ...
        return redirect(url_for('index'))  # or you can keep the form open if needed
