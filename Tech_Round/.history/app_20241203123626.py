...
def generate_combined_graph(scores_by_question):
    ...
    """
    # Prepare data
    # Sort dates by converting to datetime objects to ensure correct ordering
    dates = sorted({datetime.strptime(date, "%Y-%m-%d") for scores in scores_by_question.values() for date, _ in scores})
    ...
    # Try tight_layout first, otherwise use manual layout adjustment
    try:
        plt.tight_layout()
    except Exception as e:
        print(f"Error with tight_layout: {e}")
