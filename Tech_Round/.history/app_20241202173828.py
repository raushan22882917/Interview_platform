...
def generate_prompt(step, question, user_input):
    """Generate tailored prompts for each step with score and HTML feedback formatting."""

    def feedback_template(criteria_list):
        """Return feedback section with bullet points and scoring instructions."""
        criteria_html = ''.join(f"<li>{criterion}</li>" for criterion in criteria_list)
        return (
            f"<ul>{criteria_html}</ul>"
            f"<p><strong>Score (1 to 5):</strong> Assign based on how well the user meets the criteria above.</p>"
            f"<p><strong>Feedback:</strong> Provide up to 5 bullet points with suggestions and highlights.</p>"
            f"<ul><li>Example feedback bullet point.</li></ul>"
        )

    if step == "Example":
        criteria = [
            "Ensure at least 3 input-output pairs are provided.",
            "Check for diversity in examples.",
            "Evaluate clarity of examples.",
            "Confirm relevance to the question.",
        ]
        return (
            f"<h3>Task: Evaluate Examples</h3>"
            f"<p><strong>Question:</strong> {question}</p>"
            f"<p><strong>Provided Examples:</strong></p><pre>{user_input}</pre>"
            + feedback_template(criteria)
        )
    
    elif step == "Solution":
        criteria = [
            "Check correctness of the solution approach.",
            "Evaluate feasibility and practicality.",
            "Assess logical flow and organization.",
            "Ensure completeness in addressing the problem.",
        ]
        return (
            f"<h3>Task: Assess Solution Approach</h3>"
            f"<p><strong>Question:</strong> {question}</p>"
            f"<p><strong>Proposed Solution:</strong></p><pre>{user_input}</pre>"
            + feedback_template(criteria)
        )
        ...
        criteria = [
            ...
            "Ensure test cases thoroughly test the solution.",
            "Assess completeness of test coverage.",
        ]
        return (
            f"<h3>Task: Review Test Cases</h3>"
            f"<p><strong>Question:</strong> {question}</p>"
            f"<p><strong>Test Cases Provided:</strong></p><pre>{user_input}</pre>"
            + feedback_template(criteria)
        )
        ...
        criteria = [
            ...
            "Suggest coding improvements or best practices.",
        ]
        return (
            f"<h3>Task: Code Review</h3>"
            f"<p><strong>Question:</strong> {question}</p>"
            f"<p><strong>User's Code:</strong></p><pre>{user_input}</pre>"
            + feedback_template(criteria)
        )
    
    elif step == "Validation":
        criteria = [
            "Ensure time complexity is provided and accurate.",
            "Check space complexity details.",
            "Verify all test cases pass, both public and private.",
        ]
        return (
            f"<h3>Task: Validation Review</h3>"
            f"<p><strong>Question:</strong> {question}</p>"
            f"<p><strong>Validation Details:</strong></p><pre>{user_input}</pre>"
            + feedback_template(criteria)
        )
