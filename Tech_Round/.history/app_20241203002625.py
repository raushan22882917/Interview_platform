...
# Initialize GroqAPI client
groq_client = Groq(api_key='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')  # Replace with your actual GroqAPI key
...
def next_question():
    ...
    logging.info(f"Current index: {current_index}, Total questions: {len(selected_questions)}")
...
def submit_answer_take():
    ...
    user_email = session.get('email')  # Use 'email' session key as requested
    ...
    if current_index < len(selected_questions):
        ...
        answers = [row['Web Answer'], row['GPT3.5 Answer'], row['GPT4 Answer']]
        
        # Get embeddings and calculate similarities using GroqAPI
        similarities = []
        for answer in answers:
            response = groq_client.get_similarity(model="llama3-8b-8192", inputs=[user_answer, answer])
            similarity = response['similarity'] * 10  # Scale similarity
...
def generate_prompt(step, question, user_input):
    ...
    if step == "Example":
        return (
            ...
            f"#### Feedback:\n"
            f"- Provide qualitative feedback in 5 bullet points, using terms such as Excellent, Very Good, Good, or Needs Improvement.\n"
    ...
    # Step 5: Validation Feedback
        ...
        return (
            ...
            f"#### Evaluation Criteria:\n"
            f"- **Time Complexity:** Confirm if it is provided and accurate.\n"
            f"- **Space Complexity:** Check if it is stated correctly.\n"
            f"- **Test Case Results:** Review whether all public and private test cases pass.\n\n"
            f"#### Feedback:\n"
            f"- Provide qualitative feedback in 5 bullet points, using terms such as Excellent, Very Good, Good, or Needs Improvement.\n"
            f"- Suggestions for improvement (if any):"
        )
...
def extract_score_from_feedback(feedback):
    ...
    if "Excellent" in feedback:
        ...
        return 1  # Default for unclear/poor feedback
