...
def get_user_activity_dates(user_email):
    ...
    # Fetch created_at dates and associated scores for the user
    cursor.execute("""
    ...
    # Prepare the result as a list of dictionaries with 'date' and 'avg_score'
    attended_days = []
    for result in evaluation_results:
        score_date = result['score_date']
        avg_score = result['avg_score']
        attended_days.append({
            "date": score_date.strftime('%Y-%m-%d'),  # Ensure date is in 'YYYY-MM-DD' format
            "score": round(avg_score, 2)             # Round the average score to 2 decimal places
        })

    cursor.close()
    connection.close()

    # Return the list of attended days with their average scores
...
def profile():
    ...
    # Handle POST request for updating profile image or educational details
    if request.method == 'POST':
        ...
        # Fetch and update educational details
        ...
        cursor.execute("""
            INSERT INTO education (
                user_email, first_name, last_name, college_name, dob, 
    ...
    # Fetch the latest educational details
    cursor.execute("SELECT first_name, last_name, college_name, dob, year_of_study, graduation_year FROM education WHERE user_email = %s;", (user_email,))
   ...
   # Fetch scores for each question_id grouped by date
    ...
    return render_template('profile_details.html',
                           ...
                           attended_days=attended_days,user_scores=user_scores,graph_html=graph_html,streak_data=streak_data) 
...
def generate_combined_graph(scores_by_question):
    ...
    ax.set_title("Scores by Question ID with Average Score Line")
    ax.legend()
...
def get_user_streak_levels(email):
    ...
    for activity in user_activity:
        ...
        # Check if the score is more than 70 and the day is consecutive
        if avg_score >= 3:
            if previous_date and (activity_date - previous_date).days == 1:
                consecutive_days += 1
            else:
                consecutive_days = 1  # Reset streak if not consecutive

            # Unlock levels every 10 continuous days of high performance
            if consecutive_days == 2:
                streak_level += 1
                consecutive_days = 0  # Reset streak count after unlocking a level

        previous_date = activity_date

        # Add data to streak_data for visualization
        streak_data.append({
