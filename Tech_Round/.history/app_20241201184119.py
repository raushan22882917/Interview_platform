...
# Check subscription status
    ...
    average_scores_hr = []  # Store the average scores for each attempt
    ...
    for i in range(0, num_rows, 10):
        ...
        attempts.append({
            ...
            "average_score": round(attempt_total_score / len(attempt_rows), 2) if len(attempt_rows) > 0 else 0,
        ...
        average_scores_hr.append(round(attempt_total_score / len(attempt_rows), 2) if len(attempt_rows) > 0 else 0)

    # Calculate total and average score for all attempts
    total_score = sum(attempt["total_score"] for attempt in attempts)
    average_scores_hr = round(total_score / len(rows), 2) if len(rows) > 0 else 0
    return render_template('profile_details.html',
                           ...
                           percentage_scored_above_70=percentage_scored_above_70,attempts=attempts, total_score=total_score, average_scores_hr=average_scores_hr,dates=dates,total_scores=total_scores,average_scores=average_scores,
