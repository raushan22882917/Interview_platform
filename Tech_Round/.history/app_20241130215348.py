...
@app.route('/subscribe')
def subscribe():
    return render_template('HR_Round/subscribe.html')

@app.route('/process_payment', methods=['POST'])
def process_payment():
    subscription_type = request.form['subscription_type']
    months_map = {'1-month': 1, '3-months': 3, '6-months': 6, '12-months': 12}
    price_map = {'1-month': '0.1', '3-months': '0.2', '6-months': '0.3', '12-months': '0.4'}

    payment = paypalrestsdk.Payment({
        "intent": "sale",
        "payer": {"payment_method": "paypal"},
        "redirect_urls": {
            "return_url": url_for('payment_success', subscription_type=subscription_type, _external=True),
            "cancel_url": url_for('subscribe', _external=True)
        },
        "transactions": [{
            "item_list": {"items": [{
                "name": f"{subscription_type} subscription",
                "sku": "subscription",
                "price": price_map[subscription_type],
                "currency": "USD",
                "quantity": 1
            }]},
            "amount": {"total": price_map[subscription_type], "currency": "USD"},
            "description": f"{subscription_type} access to interview platform"
        }]
    })
...
def payment_success():
    ...
    user_email = session['email']
    months = {'1-month': 1, '3-months': 3, '6-months': 6, '12-months': 12}[subscription_type]
    end_date = datetime.datetime.now() + datetime.timedelta(days=30 * months)

    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute(
        """
        INSERT INTO subscriptions (user_email, subscription_type, end_date)
        VALUES (%s, %s, %s)
        """, (user_email, subscription_type, end_date)
    )
    connection.commit()
    cursor.close()
    connection.close()

    return jsonify({'message': 'Subscription successful! You now have full access.'})
@app.route('/process_subscription', methods=['GET'])
def process_subscription():
    ...
    # Logic to update user's subscription in the database
    if duration == '1 Month':
        # Update user's subscription to 1 month
        pass
    elif duration == '3 Months':
        # Update user's subscription to 3 months
        pass
    elif duration == '6 Months':
        # Update user's subscription to 6 months
        pass
    elif duration == '12 Months':
        # Update user's subscription to 12 months
        pass

    # Return a success response or redirect to a success page
    return jsonify({
        'message': f'Your {duration} subscription has been activated successfully.'
    })
