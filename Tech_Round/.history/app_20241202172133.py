...
def generate_prompt(step, question, user_input):
    """Generate tailored prompts for each step with a score."""

    if step == "Example":
        content = (
            f"Question: {question}\nExamples provided: {user_input}\n"
            f"Check if at least 3 input-output pairs are provided and assess diversity, clarity, and relevance.\n"
            f"Provide a score (1 to 5) for the examples provided based on the criteria."
        )
    
    elif step == "Solution":
        content = (
            f"Question: {question}\nSolution approach: {user_input}\n"
            f"Evaluate the approach used based on correctness, feasibility, logical flow, and completeness.\n"
            f"Provide a score (1 to 5) for the solution approach based on the evaluation."
        )
    
    elif step == "Test Cases":
        content = (
            f"Question: {question}\nTest cases: {user_input}\n"
            f"Check if the user covers all possible scenarios, including edge cases, and provide feedback on completeness.\n"
            f"Provide a score (1 to 5) for the test cases based on the evaluation."
        )
    
    elif step == "Code":
        content = (
            f"Question: {question}\nUser's code: {user_input}\n"
            f"Evaluate the code in terms of correctness, time complexity, space complexity, and suggest improvements if needed.\n"
            f"Provide a score (1 to 5) for the code based on the evaluation."
        )
    
    elif step == "Validation":
        content = (
            f"Question: {question}\nValidation details: {user_input}\n"
            f"Check if the user provided time complexity, space complexity, and automatically ran all public and private test cases. Provide feedback on whether all test cases passed.\n"
            f"Provide a score (1 to 5) for the validation details based on the evaluation."
        )
    
    # Prepare the request in the correct format
    messages = [
        {"role": "system", "content": "You are an evaluator providing feedback and scores for responses."},
        {"role": "user", "content": content}
    ]

    feedback = "Excellent"  # This would be dynamically generated by the Groq API.
    score = extract_score_from_feedback(feedback)
    
    return messages, feedback, score
