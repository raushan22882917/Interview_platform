...
@app.route('/subscribe')
def subscribe():
    return render_template('subscription.html')  # Update this with your actual HTML file
        ...
        "transactions": [{
            "item_list": {
                "items": [{
                    "name": "Subscription",
                    "sku": "12345",
                    "price": "0.10",  # This should be dynamic based on user selection
                    "currency": "USD",
                    "quantity": 1
                }]
            },
            "amount": {
                "total": "0.10",  # This should be dynamic
                "currency": "USD"
            },
            "description": "Subscription to interview platform"
        }]
    })
...
def execute():
    ...
    if payment.execute({'payer_id': request.form['payerID']}):
        success = True
        # Store payment details in the database and mark user as subscribed
        email = request.form['email']  # You would get the email from the user session or form
        uid = request.form['uid']  # Get the UID from the form

        # Store subscription info in the database
        connection = get_db_connection()
        cursor = connection.cursor()
        cursor.execute("""
            INSERT INTO subscriptions (email, uid, payment_id, subscription_date)
            VALUES (%s, %s, %s, NOW())
        """, (email, uid, payment.id))
        connection.commit()
        cursor.close()
        connection.close()
...
def submit_uid():
    ...
    return jsonify({'message': 'UID submitted successfully'})
