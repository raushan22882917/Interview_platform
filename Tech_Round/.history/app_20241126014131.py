...
def generate_full_year_calendar(user_email):
    ...
    while current_date <= end_date:
        ...
        # Move to the next day, adjusting for the number of days in the current month
        # Get the number of days in the current month (taking into account leap years)
        num_days_in_month = calendar.monthrange(current_date.year, current_date.month)[1]

        if day < num_days_in_month:
            current_date = current_date.replace(day=day + 1)
        else:
...
""", (user_email, first_name, last_name, college_name, dob, year_of_study, graduation_year, linkedin_url, instagram_url))
   ...
   # Fetch scores for each question_id grouped by date
    ...
    activity_data = generate_full_year_calendar(user_email)
