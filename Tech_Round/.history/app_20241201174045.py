...
# Function to generate PDFs and calculate total score for each attempt
def generate_pdfs(rows, rows_per_pdf=10):
    ...
    for i in range(0, len(rows), rows_per_pdf):
        ...
        pdf.cell(40, 10, "Company Name", border=1)
        pdf.cell(80, 10, "Question", border=1)
        pdf.cell(30, 10, "User Answer", border=1)
        pdf.cell(30, 10, "Feedback", border=1)
        ...
        for row in rows[i:i + rows_per_pdf]:
            ...
            pdf.cell(40, 10, clean_text(company_name), border=1)
            pdf.cell(80, 10, clean_text(question), border=1)
            pdf.cell(30, 10, clean_text(user_answer), border=1)
            pdf.cell(30, 10, clean_text(feedback), border=1)
            pdf.cell(20, 10, str(score), border=1)
            pdf.ln()
        ...
        pdf.ln(5)
        pdf.set_font("Arial", style="B", size=12)
        pdf.cell(0, 10, f"Total Score: {total_score}", ln=True, align="C")

        # Save the PDF
        pdf_path = os.path.join(PDF_FOLDER, f"Attempt_{pdf_index}.pdf")
        pdf.output(pdf_path)

        # Add attempt info for display
        attempt_info = {
            "id": pdf_index,
