...
def store_in_db(company_name, question, user_answer, feedback, score, user_email):
    ...
    # Step 1: Check if the user has more than 2 entries in the interview_data table
    cursor.execute(
        """
        SELECT COUNT(*) FROM interview_data WHERE user_email = %s
        """, (user_email,)
    )
    interview_count = cursor.fetchone()[0]

    if interview_count >= 2:
        # Step 2: Check if the email exists in the payments table
        cursor.execute(
            """
            SELECT amount, date, months FROM payments WHERE email = %s
            """, (user_email,)
        )
        payment_data = cursor.fetchone()

        if payment_data is None:
            # Step 3: If the email does not exist in the payments table, show a message
            cursor.close()
            connection.close()
            return "Subscription not taken."

        # Step 4: If the email exists, check if the subscription is active
        amount, payment_date, months = payment_data
        payment_date = datetime.strptime(payment_date, '%Y-%m-%d')  # Assuming date is in 'YYYY-MM-DD' format
        subscription_end_date = payment_date.replace(year=payment_date.year + months // 12, month=(payment_date.month + months) % 12)

        # Calculate the remaining days of the subscription
        days_left = (subscription_end_date - datetime.now()).days

        if days_left <= 0:
            # Step 5: If subscription is expired, show a message
            cursor.close()
            connection.close()
            return "Subscription expired. Please renew your subscription."

        # If subscription is still active, proceed to save the interview data
        cursor.execute(
            """
        ...
        return f"Subscription active. {days_left} days left."
    
    else:
        # If there are less than 2 entries, proceed with saving the interview data
        cursor.execute(
            """
            INSERT INTO interview_data (company_name, question, user_answer, feedback, score, user_email)
            VALUES (%s, %s, %s, %s, %s, %s)
        ...
        return "Interview data saved successfully."

@app.route('/hr_index', methods=['POST'])
def hr_index():
    ...
    # Fetch data from the form or request
    ...
    # Call store_in_db and get the response message
    response_message = store_in_db(company_name, question, user_answer, feedback, score, user_email)

    # Pass the email and response message to the template for rendering
    return render_template('HR_Round/index.html', user_email=user_email, response_message=response_message)
...
def submit_answer():
    ...
    # Just return an acknowledgment without feedback or score
    return jsonify({
