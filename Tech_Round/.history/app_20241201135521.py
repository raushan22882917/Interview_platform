...
def store_in_db(company_name, question, user_answer, feedback, score, user_email):
    ...
    cursor.execute("SELECT date, months FROM payments WHERE email = %s", (user_email,))
    payment_data = cursor.fetchone()

    if payment_data:
        # Subscription is found, calculate expiration date
        payment_date = payment_data[0]
        subscription_months = payment_data[1]
        expiration_date = payment_date + timedelta(days=subscription_months * 30)  # Assuming 30 days per month

        # Check if the subscription is still active
        if datetime.now() < expiration_date:
            # Subscription is valid, insert interview data
            cursor.execute("""
        ...
        # Email not found in payments table
        message = "Subscription not taken. Please subscribe to continue."
...
def hr_index():
    ...
    # Get the form data from the request (this can include the interview data)
    ...
    # Call store_in_db function to insert data and get the result message
    message, button_text = store_in_db(company_name, question, user_answer, feedback, score, user_email)

    # Pass the message and button text to the template for rendering
    return render_template('HR_Round/index.html', user_email=user_email, message=message, button_text=button_text)
