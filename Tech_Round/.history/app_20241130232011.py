...
def has_valid_subscription(user_email):
    connection = get_db_connection()
    cursor = connection.cursor()
    cursor.execute(
        """
        SELECT end_date FROM subscriptions
        WHERE user_email = %s AND end_date > CURRENT_TIMESTAMP
        """, (user_email,)
    )
...
def evaluate_answer_hr(user_answer, question_text):
    chat_completion = client.chat.completions.create(
        messages=[
            {
                "role": "system",
                "content": "Evaluate the user's interview answer."
            },
            {
                "role": "user",
...
def submit_answer():
    ...
    cursor.execute(
        """
        SELECT COUNT(*) FROM interview_data WHERE user_email = %s AND DATE(created_at) = CURRENT_DATE
        """, (user_email,)
    )
    question_count = cursor.fetchone()[0]
    
    # Allow 2 free questions per day
    if question_count >= 2 and not has_valid_subscription(user_email):
        return redirect(url_for('subscribe'))

    company_name = request.form['company_name']
    question_index = int(request.form['question_index'])
    user_answer = request.form['user_answer']
    question_text = format_questions(company_name)[question_index]

    feedback = evaluate_answer_hr(user_answer, question_text)
    score = CATEGORY_SCORES.get(feedback.split()[0], 2)
...
def get_db_connection():
    return psycopg2.connect(
        ...
        password="22882288"    # Database password
    )

# Function to generate QR code for UPI payment
def generate_qr_code(subscription_type):
    # Create the UPI URL for the selected subscription type
    upi_url = ""
    if subscription_type == '1-month':
        upi_url = "upi://pay?pa=merchant@upi&pn=MerchantName&am=0.1&cu=INR"
    elif subscription_type == '3-months':
        upi_url = "upi://pay?pa=merchant@upi&pn=MerchantName&am=0.2&cu=INR"
    elif subscription_type == '6-months':
        upi_url = "upi://pay?pa=merchant@upi&pn=MerchantName&am=0.3&cu=INR"
    elif subscription_type == '12-months':
        upi_url = "upi://pay?pa=merchant@upi&pn=MerchantName&am=0.4&cu=INR"
    
    # Generate QR code for the UPI URL
    img = qrcode.make(upi_url)
    qr_code_path = f"static/qr_codes/{subscription_type}.png"
    img.save(qr_code_path)

    return qr_code_path

# Route for displaying the subscription page
@app.route('/subscribe')
def subscribe():
    return render_template('subscribe.html')

# Route for generating the QR code based on the selected subscription type
@app.route('/generate_qr_code', methods=['POST'])
def generate_qr_code_route():
    subscription_type = request.form['subscription_type']  # Get selected subscription type
    qr_code_path = generate_qr_code(subscription_type)      # Generate QR code

    return jsonify({
        'qr_code_url': qr_code_path  # Return the path of the generated QR code
    })

# Route to handle payment confirmation and UID submission
@app.route('/submit_payment', methods=['POST'])
def submit_payment():
    uid = request.form['uid']  # Get the UID entered by the user after payment
    subscription_type = request.form['subscription_type']  # Get the selected subscription type
    user_email = session.get('email')  # Get user email from session (assuming user is logged in)

    if not user_email:
        return jsonify({'message': 'User not logged in'}), 400  # Return error if user is not logged in
    ...
    if subscription_type == '1-month':
        ...
        end_date = 'NOW() + INTERVAL \'12 months\''

    # Insert subscription data into the database
    cursor.execute("""
        INSERT INTO subscriptions (user_email, subscription_type, end_date, uid)
        VALUES (%s, %s, %s, %s)
    """, (user_email, subscription_type, end_date, uid))

    connection.commit()
    cursor.close()
    connection.close()

    return jsonify({'message': 'Payment Successful and Subscription Activated'})
