...
def start_quiz(position_name):
    ...
    try:
        ...
        return f"Error loading questions: {e}", 500
    
    if not questions:
        return "No questions found in the CSV file.", 404

    # Randomly select 5 questions (or fewer if the file has less than 5 questions)
    selected_questions = random.sample(questions, min(5, len(questions)))

    # Save questions and index to the session
...
def next_question():
    ...
    print("Selected Questions:", selected_questions)  # Debugging log
    print("Current Index:", current_index)  # Debugging log

    if not selected_questions:
        return jsonify({'error': 'No questions available. Please restart the quiz.'}), 400

    if current_index < len(selected_questions):
        question = selected_questions[current_index]['Question']
        session['current_index'] += 1  # Increment the index
