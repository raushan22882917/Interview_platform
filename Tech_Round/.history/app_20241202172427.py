...
def generate_prompt(step, question, user_input):
    """Generate tailored prompts for each step."""
    if step == "Example":
        return (
            f"Question: {question}\nExamples provided: {user_input}\n"
            f"Check if at least 3 input-output pairs are provided and assess diversity, clarity, and relevance.and give score in 1 to 5"
        )
    
    elif step == "Solution":
        return (
            f"Question: {question}\nSolution approach: {user_input}\n"
            f"Evaluate the approach used based on correctness, feasibility, logical flow, and completeness.and give score in 1 to 5"
        )
    
    elif step == "Test Cases":
        return (
            f"Question: {question}\nTest cases: {user_input}\n"
            f"Check if the user covers all possible scenarios, including edge cases, and provide feedback on completeness.and give score in 1 to 5"
        )
    
    elif step == "Code":
        return (
            f"Question: {question}\nUser's code: {user_input}\n"
            f"Evaluate the code in terms of correctness, time complexity, space complexity, and suggest improvements if needed.and give score in 1 to 5"
        )
    
    elif step == "Validation":
        return (
            f"Question: {question}\nValidation details: {user_input}\n"
            f"Check if the user provided time complexity, space complexity, and automatically run all public and private test cases. Provide feedback on whether all test cases passed.and give score in 1 to 5"
        )
