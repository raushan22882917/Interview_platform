...
def start():
    ...
    return render_template('Ai/steps.html', step=STEPS[0], question=session['question'])
...
def process_step():
    ...
    if current_index < len(STEPS) - 1:
        ...
        return render_template('Ai/steps.html', step=next_step, question=session['question'])
    else:
        session['current_step'] = None  # All steps are complete
        return render_template('Ai/final_feedback.html', feedback=session)
