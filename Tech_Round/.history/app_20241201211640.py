...
def generate_combined_graph(scores_by_question):
    ...
    and a wave-like line chart connecting average scores per date.
    """
    ...
    fig, ax = plt.subplots(figsize=(max(len(dates) * 2, 12), 6))
    bar_width = 0.8 / len(question_ids)  # Normalized bar width for even spacing
    ...
    # Smooth wave-like line for average scores
    x_smooth = np.linspace(x_positions.min(), x_positions.max(), 300)
    avg_smooth = make_interp_spline(x_positions, avg_scores)(x_smooth)
    ax.plot(
        x_smooth,
        avg_smooth,
        color="red",
