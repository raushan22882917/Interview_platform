...
def evaluate_answer_hr(user_answer, question_text):
    ...
    # Extract the category from feedback (assuming category is a single word within feedback)
    for category in CATEGORY_SCORES.keys():
        if category in feedback:
            return feedback, CATEGORY_SCORES[category]
    
    # Default return if no category found
    return feedback, 1
...
def submit_answer():
    ...
    feedback, score = evaluate_answer_hr(user_answer, question_text)
